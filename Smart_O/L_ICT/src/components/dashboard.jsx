...
function Dashboard() {
  ...
  const [totalLessons, setTotalLessons] = useState(100);
  ...
  const fetchDashboardData = () => {
    ...
    Promise.all([
      ...
      .then(async ([g10Res, g11Res, progressRes]) => {
        ...
        const activities = Array.isArray(progressRecords) ? progressRecords
          .filter(p => p.completed || p.score !== undefined)
          .sort((a, b) => new Date(b.updatedAt || b.createdAt || 0) - new Date(a.updatedAt || a.createdAt || 0))
          .slice(0, 5) // Take top 5 recent activities
          .map(p => ({
            text: p.completed ? `Completed lesson: ${p.lesson_title || 'Unknown Lesson'}` : `Scored ${p.score}% in quiz`,
...
const addRecentActivity = (text) => {
  ...
  setRecentActivities(prev => [newActivity, ...prev.slice(0, 4)]); // Keep only 5 activities
};
