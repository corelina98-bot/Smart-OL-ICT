...
export default function ProgressTracking() {
  ...
  const [manualProgressGrade10, setManualProgressGrade10] = useState(() => {
    const saved = localStorage.getItem('manualProgressGrade10');
    return saved ? parseInt(saved, 10) : 0;
  });
  const [manualLessonProgress, setManualLessonProgress] = useState(() => {
    const saved = localStorage.getItem('manualLessonProgress');
    return saved ? JSON.parse(saved) : {};
  });
  const [isGrade10Clicked, setIsGrade10Clicked] = useState(false);
  const [clickedLessons, setClickedLessons] = useState({});

  const matchesLesson = (p, lesson) => {
    if (!p || !lesson) return false;
    const pid = p.lesson || p.lesson_id || p.lessonId || p.lessonId;
    ...
    Promise.all(fetchPromises)
      .then(async (responses) => {
        ...
        const completed10 = total10 > 0
          ? lessons10.filter(lesson => progress.some(p => matchesLesson(p, lesson) && p.completed)).length
          : 0;
        const completed11 = total11 > 0
          ? lessons11.filter(lesson => progress.some(p => matchesLesson(p, lesson) && p.completed)).length
          : 0;
  ...
  // Persist manual progress to localStorage
  useEffect(() => {
    localStorage.setItem('manualProgressGrade10', manualProgressGrade10);
  }, [manualProgressGrade10]);

  useEffect(() => {
    localStorage.setItem('manualLessonProgress', JSON.stringify(manualLessonProgress));
  }, [manualLessonProgress]);
              ...
              <div className="progress-and-total">
                <div className="progress-circle-container">
                  <div className="progress-circle">
                    ...
                    <div className="circle-fill" style={{ transform: `rotate(${manualProgressGrade10 * 3.6}deg)` }}></div>
                    <div className="circle-text">{manualProgressGrade10}%</div>
                  </div>
                </div>
                <div className="pt-total">Total {grade10Total}</div>
              </div>
            </div>
  ...
  <div className="progress-and-total">
    <div className="progress-circle-container">
      <div className="progress-circle">
        ...
        <div className="circle-text">{grade11Total>0? Math.round((grade11Completed/grade11Total)*100):0}%</div>
      </div>
    </div>
    <div className="pt-total">Total {grade11Total}</div>
  </div>
</div>
                  ...
                  <div className="pt-col">
                    ...
                    <ul>
                      {lessons10.map((lesson, index) => {
                        ...
                        const manualPercent = manualLessonProgress[lessonId] || 0;
                        const progressEntry = progressData.find(p => matchesLesson(p, lesson));
                        const lessonScore = progressEntry && progressEntry.completed ? 0 : 0;
                        const quizScore = progressEntry ? (progressEntry.completed ? 0 : 0) : 0;
 
                        const totalScore = lessonScore + quizScore;
                        const lessonWidth = totalScore > 0 ? (lessonScore / totalScore) * 100 : 50;
                        const quizWidth = totalScore > 0 ? (quizScore / totalScore) * 100 : 50;
                        ...
                        return (
                          <li key={lesson._id || index}>
                            ...
                            <div className="lesson-progress-info">
                              <span className="progress-percent">Lesson: {Math.round(lessonScore)}% | Quiz: {Math.round(quizScore)}%</span>
