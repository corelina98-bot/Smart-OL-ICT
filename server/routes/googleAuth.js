  ...
  async (req, res) => {
    try {
      ...
      if (authType === "register") {
        if (existingUser) {
          // Email already exists - automatically link Google to existing account
          ...
          // Generate JWT for linked account
          ...
          return res.redirect(`http://localhost:5173/login?token=${token}&message=google_linked`);
        }
      ...
      if (authType === "login") {
        ...
        // Reset progress for the user on login
        await ProgressReport.deleteMany({ user: existingUser._id });
